...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
step_time=`grep -a 'time:' ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log| awk -F 'time: ' '{print $2}'|awk -F ',' '{print $1}'|tail -n 31|head -n 30|awk '{sum += $1} END {print sum/NR}'`
