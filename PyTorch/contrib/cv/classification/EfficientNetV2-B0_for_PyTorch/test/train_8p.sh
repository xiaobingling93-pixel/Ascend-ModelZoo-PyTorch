...
DeviceType=`uname -m`
CaseName=${Network}_bs${BatchSize}_${RANK_SIZE}'p'
...
if (($performance==0));then
    #输出训练精度,需要模型审视修改
  train_accuracy=`grep -a '*** Best metric: ' ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk 'END {print}'|awk '{print $(NF-2)}'`
  #打印，不需要修改
