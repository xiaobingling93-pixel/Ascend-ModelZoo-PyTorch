...
class FlashSelfAttention(torch.nn.Module):
    ...
    def forward(self, qkvn, attention_mask):
        ...
        if self.causal:
            ...
            return output
        raise Exception("the attention type {} is not support!")
...
class TelechatAttention(nn.Module):
    ...
    def forward(
        ...
        if self.config.flash_attn:
            atten_mask_ = torch.triu(torch.ones(s,s),1).to(torch.float)
            atten_flash_mask = atten_mask_.clone().bool().to(query_layer.device)
            ...
            context_layer = self.core_attention_flash((q, k, v, self.num_heads), atten_flash_mask)
...
class TelechatModel(TelechatPreTrainedModel):
    ...
    def forward(
            ...
            past_key_values: Optional[Tuple[Tuple[torch.Tensor, torch.Tensor], ...]] = None,            attention_mask: Optional[torch.Tensor] = None,
        ...
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions        output_hidden_states = (
