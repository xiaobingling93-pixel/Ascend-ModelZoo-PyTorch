...
NGPUS=1
Network="bisenetv2"
CaseName=${Network}_bs${BatchSize}_${NGPUS}p_accuracy
...
torchrun --nproc_per_node=$NGPUS tools/train_amp.py --config configs/bisenetv2_city.py --mode accuracy >test/output/train.log 2>&1
...
TrainAccuracy=`grep -a '| mious' test/output/train.log | awk -F "|" '{print $4}' | sed s/[[:space:]]//g | awk '{print $1}'`
...
echo "TrainAccuracy = ${TrainAccuracy}" >>test/output/${CaseName}.log
