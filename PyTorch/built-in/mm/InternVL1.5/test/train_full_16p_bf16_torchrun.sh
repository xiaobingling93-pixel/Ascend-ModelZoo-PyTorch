...
if [ -z "MASTER_ADDR" ]; then
  echo "MASTER_ADDR MUST BE SET"
  exit 1
fi

if [ -z "NODE_RANK" ]; then
  echo "NODE_RANK MUST BE SET"
  exit 1
fi
...
torchrun \
  ...
  > ${output_path}/train_${NODE_RANK}.log 2>&1 &
...
echo "------------------ Final result ------------------"
# 输出性能FPS，需要模型审视修改
FPS=`grep -a 'train_samples_per_second =' ${output_path}/train_${NODE_RANK}.log | awk -F "train_samples_per_second =" '{print $2}' | sed 's/[[:space:]]//g'`
# 打印，不需要修改
...
# 从train_${NODE_RANK}.log提取Loss到train_${CaseName}_loss.txt中，需要模型审视修改
grep {\'loss\': ${output_path}/train_${NODE_RANK}.log | awk -F "{'loss': " '{print $NF}' | awk -F "," '{print $1}' > ${output_path}/train_${CaseName}_loss.txt

# 最后一个迭代loss值，不需要修改
ActualLoss=`grep -a 'train_loss' ${output_path}/train_${NODE_RANK}.log | awk 'NR==2 {print}' |awk -F "=" '{print $2}' | sed 's/[[:space:]]//g'`
