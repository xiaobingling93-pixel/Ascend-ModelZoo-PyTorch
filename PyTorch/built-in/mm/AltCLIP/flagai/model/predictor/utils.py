...
def encode(tokenizer, i, target_span_len=100, use_target=False):
    ...
    if task == 0:
        # Continue writing (only predict single span)
        if len(i['source']) > 1 and len(i['source'][1]) > 0:
            # Generate middle part, the context and eos afterwards and be seen
            ...
            ids += tgt_ids
            # Option1: The model generates eos
            info.extend([len(tgt_ids), 0])
            # Option2: length is fixed and eos is provided
            # info.extend([len(tgt_ids)-1, 1])
    elif task == 1 or task == 2 or task == 5 or task == 6:
        # Extraction and continue writing(sentence and paragraph levels)
        ...
        tgt_ids = [tokenizer.bos_id] + tgt_ids + [tokenizer.eos_id]
        ids += tgt_ids
        # length not controled, but generated by eos
        info.extend([len(tgt_ids), 0])
...
def make_input_cpm3(ctx, info, prompt_length):
    ...
    _span[len_inp] = 1  # The next digit of the end of each concatenated sentence is 1
