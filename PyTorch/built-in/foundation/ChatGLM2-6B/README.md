...
- 当前模型支持的 PyTorch 版本和已知三方库依赖如下表所示。
  ...
  **表 1**  版本支持表
  ...
  | :--------: | :----------------------------------------------------------: |
  | PyTorch 1.11 |transformers == 4.29.0; deepspeed == 0.9.2 |
...
USE_FLASH=False
```
4. 任务评估过程中出现`XXXX does not appear to have a file named tokenization_chatglm.py/ configuration_chatglm.py / modeling_chatglm.py`时，将`model`路径下的`tokenization_chatglm.py/ configuration_chatglm.py / modeling_chatglm.py`拷贝到`XXXX`路径下。

5. 任务评估过程中如果报错`RuntimeError: call aclnnFlashAttentionScore failed`，请修改`XXXX`路径下的`modeling_chatglm.py`。
   ```python
   # 修改modeling_chatglm.py文件:
   USE_FLASH=False
   ```
6. 如果出现报错：ValueError: FP16 Mixed precision training with AMP or APEX (`--fp16`) and FP16 half precision evaluation (`--fp16_full_eval`) can only be used on CUDA devices.
请在`transformers/training_args.py`文件的1337行将报错信息注释掉。
