...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="Qwen2_5_vl_7b_for_PyTorch"
...
nohup python3 -m verl.trainer.main_ppo \
    ...
    trainer.save_freq=-1 \
    trainer.test_freq=-1 \
    trainer.total_epochs=15 > ${test_path_dir}/output/train_verl_qwen2_5_vl_7b.log 2>&1 &
wait
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep 'perf/throughput:' $test_path_dir/output/train_verl_qwen2_5_vl_7b.log | awk -F 'perf/throughput:' '{print$2}' | awk -F ' ' '{print$1}' | head -n 4 | awk '{sum+=$1} END {print"",sum/NR}'`
