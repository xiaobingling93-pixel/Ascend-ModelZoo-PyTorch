...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="Qwen2_5_vl_32b_for_PyTorch"

# 帮助信息，不需要修改
if [[ $1 == --help || $1 == -h ]];then
    echo"usage:./test/train_qwen2_5_vl_32b_GRPO_full_32p.sh <args>"
    echo " "
...
nohup python3 -m verl.trainer.main_ppo \
    ...
    trainer.total_epochs=15 > ${test_path_dir}/output/train_verl_qwen2_5_vl_32b.log 2>&1 &
wait
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep 'perf/throughput:' $test_path_dir/output/train_verl_qwen2_5_vl_32b.log | awk -F 'perf/throughput:' '{print$2}' | awk -F ' ' '{print$1}' | head -n 4 | awk '{sum+=$1} END {print"",sum/NR}'`
