...
if [[ ${1} != "slurm" ]]; then
  deepspeed --module $training_commands > ${cur_path}/test/output/${RL_ALGO}/train_${RL_ALGO}.log 2>&1 &
fi
...
# 获取训练日志
source_log_file="${cur_path}/test/output/${RL_ALGO}/train_${RL_ALGO}.log"
target_log_file="${cur_path}/test/output/${RL_ALGO}/train_${RL_ALGO}_loss.log"

max_lines=1000

grep -oP "{'train/kto_loss':.*}" "$source_log_file" > "$target_log_file"

#计算平均单步耗时，默认1000步（去除前100步）
tps=$(grep -a "'train/kto_loss':" "$target_log_file" |
...
echo "RL_ALGO = ${RL_ALGO}" > ${cur_path}/test/output/${RL_ALGO}/${CaseName}.log
echo "Second Per Step = ${tps}" > ${cur_path}/test/output/${RL_ALGO}/${CaseName}.log
echo "-------------------End KTO Train-------------------"