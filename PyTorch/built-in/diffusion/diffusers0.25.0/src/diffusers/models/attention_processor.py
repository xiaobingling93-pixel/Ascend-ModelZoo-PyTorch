...
class Attention(nn.Module):
    ...
    def set_use_npu_svd_attention(
        self, use_npu_svd_attention: bool, attention_op: Optional[Callable] = None
    ) -> None:
        processor = NpuSVDAttnProcessor2_0()
        self.set_processor(processor)
...
class NpuSVDAttnProcessor2_0:
    ...
    def __init__(self):
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("NpuSVDAttnProcessor2_0 requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0.")
    ...
    def __call__(
        ...
        if query.dtype in (torch.float16, torch.bfloat16):
            if query.shape[0] > 2048:
                chunk_size = min(2048 // 2, query.shape[0])
                chunk_num = query.shape[0] // chunk_size
                key_chunk = torch.chunk(key, chunk_num, dim=0)
                value_chunk = torch.chunk(value, chunk_num, dim=0)
                query_chunk = torch.chunk(query, chunk_num, dim=0)

                hidden_states_chunk = []
                for i in range(chunk_num):
                    hidden_states_i = torch_npu.npu_fusion_attention(
                        query_chunk[i], key_chunk[i], value_chunk[i], attn.heads, input_layout="BNSD",
                        pse=None,
                        atten_mask=attention_mask,
                        scale=1.0 / math.sqrt(query.shape[-1]),
                        pre_tockens=65536,
                    ...
                    hidden_states_chunk.append(hidden_states_i)
                hidden_states = torch.cat(hidden_states_chunk, dim=0)
            else:
                hidden_states = torch_npu.npu_fusion_attention(
                    query, key, value, attn.heads, input_layout="BNSD",
                    pse=None,
