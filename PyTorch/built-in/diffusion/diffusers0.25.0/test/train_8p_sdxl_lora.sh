...
accelerate launch --config_file ./test/lora_controlnet_accelerate_config.yaml  ./examples/text_to_image/train_text_to_image_lora_sdxl.py \
...
#输出性能FPS，需要模型审视修改
FPS=$(grep "FPS: " ${output_path}/train_${mixed_precision}_sdxl_lora.log | awk '{print $NF}' | sed -n '100,199p' | awk '{a+=$1}END{print a/NR}')
...
#loss值，不需要修改
ActualLoss=$(grep -o "step_loss=[0-9.]*" ${output_path}/train_${mixed_precision}_sdxl_lora.log | awk 'END {print $NF}')
