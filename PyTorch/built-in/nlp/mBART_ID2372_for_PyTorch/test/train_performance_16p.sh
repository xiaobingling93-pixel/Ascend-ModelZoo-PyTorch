...
echo "------------------ Final result ------------------"
#输出性能WPS，需要模型审视修改
WPS=`grep 'train_inner ' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|tail -10|awk -F "wps=" '{print $NF}'|awk -F "wps" '{print $1}'|awk -F "," '{print $1}'|awk '{a+=$1} END {if (NR != 0) printf("%.3f",a/NR)}'`
