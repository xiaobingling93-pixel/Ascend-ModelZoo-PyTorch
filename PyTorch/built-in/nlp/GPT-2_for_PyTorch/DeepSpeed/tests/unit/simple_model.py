...
class SimpleFrozenModel(torch.nn.Module):
    ...
    def __init__(self, hidden_dim, empty_grad=False):
        ...
        self.linears = torch.nn.ModuleList(
            [torch.nn.Linear(hidden_dim, hidden_dim) for i in range(2)]
        )
        if empty_grad:
            self.linear2 = torch.nn.Linear(hidden_dim, hidden_dim)
        self.cross_entropy_loss = torch.nn.CrossEntropyLoss()
...
class LinearStackPipe(PipelineModule):
    ...
    def __init__(
        self, input_dim=128, hidden_dim=128, output_dim=128, num_layers=4, **kwargs
    ):
        self.input_dim = input_dim
        self.output_dim = output_dim
        self.hidden_dim = hidden_dim
        ...
        for x in range(self.num_layers):
            layers.append(
                LayerSpec(torch.nn.Linear, self.hidden_dim, self.hidden_dim, bias=False)
            )
...
def sequence_dataloader(
    model, total_samples, hidden_dim, device, seq_len: int = 32, dtype=torch.half
):
    batch_size = model.train_micro_batch_size_per_gpu()
