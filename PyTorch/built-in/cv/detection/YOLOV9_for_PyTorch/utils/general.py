...
def init_seeds(seed=0, deterministic=False):
    ...
    # torch.backends.cudnn.benchmark = True  # AutoBatch problem
...
def check_online():
    ...
    def run_once():
        # Check once
        try:
            # socket.create_connection(("x.x.x.x", 443), 5)  # check host accessibility
            return False
        except OSError:
            return False
...
def check_amp(model):
    ...
    try:
        ...
        LOGGER.warning(f'{prefix}checks failed ❌, disabling Automatic Mixed Precision.')
        return False
