...
KERNEL_NUM=$(($(nproc)/16))
for((RANK_ID=0;RANK_ID<16;RANK_ID++));
do
    export RANK_SIZE=16
...
echo "------------------ Final result ------------------"
# 输出性能FPS，需要模型审视修改
step_time=`grep "Epoch" ${test_path_dir}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log | awk -F "Time " '{print $2}' | awk -F " " '{print $3}' | awk -F ")" '{print $1}' | tail -n 1`
