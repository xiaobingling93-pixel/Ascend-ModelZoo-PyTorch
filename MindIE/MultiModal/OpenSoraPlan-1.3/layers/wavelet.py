...
class HaarWaveletTransform3D(nn.Module):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.h_conv = PlanCausalConv3d(1, 1, 2, padding=0, stride=2, bias=False)
        self.g_conv = PlanCausalConv3d(1, 1, 2, padding=0, stride=2, bias=False)
        self.hh_conv = PlanCausalConv3d(1, 1, 2, padding=0, stride=2, bias=False)
        self.gh_conv = PlanCausalConv3d(1, 1, 2, padding=0, stride=2, bias=False)
        self.h_v_conv = PlanCausalConv3d(1, 1, 2, padding=0, stride=2, bias=False)
        self.g_v_conv = PlanCausalConv3d(1, 1, 2, padding=0, stride=2, bias=False)
        self.hh_v_conv = PlanCausalConv3d(1, 1, 2, padding=0, stride=2, bias=False)
        self.gh_v_conv = PlanCausalConv3d(1, 1, 2, padding=0, stride=2, bias=False)
    ...
    def forward(self, x):
        if x.dim() != 5:
            raise ValueError(f"x.dim() should be 5")

        b = x.shape[0]
        x = rearrange(x, "b c t h w -> (b c) 1 t h w")
        rearrange_method = "(b c) 1 t h w -> b c t h w"
        low_low_low = self.h_conv(x)
        low_low_low = rearrange(low_low_low, rearrange_method, b=b)
        low_low_high = self.g_conv(x)
        low_low_high = rearrange(low_low_high, rearrange_method, b=b)
        low_high_low = self.hh_conv(x)
        low_high_low = rearrange(low_high_low, rearrange_method, b=b)
        low_high_high = self.gh_conv(x)
        low_high_high = rearrange(low_high_high, rearrange_method, b=b)
        high_low_low = self.h_v_conv(x)
        high_low_low = rearrange(high_low_low, rearrange_method, b=b)
        high_low_high = self.g_v_conv(x)
        high_low_high = rearrange(high_low_high, rearrange_method, b=b)
        high_high_low = self.hh_v_conv(x)
        high_high_low = rearrange(high_high_low, rearrange_method, b=b)
        high_high_high = self.gh_v_conv(x)
        high_high_high = rearrange(high_high_high, rearrange_method, b=b)
...
class InverseHaarWaveletTransform3D(nn.Module):
    ...
    def forward(self, coeffs):
        if coeffs.dim() != 5:
            raise ValueError(f"coeffs.dim() should be 5")
        b = coeffs.shape[0]
        ...
        rearrange_method = "b c t h w -> (b c) 1 t h w"
        low_low_low = rearrange(low_low_low, rearrange_method)
        low_low_high = rearrange(low_low_high, rearrange_method)
        low_high_low = rearrange(low_high_low, rearrange_method)
        low_high_high = rearrange(low_high_high, rearrange_method)
        high_low_low = rearrange(high_low_low, rearrange_method)
        high_low_high = rearrange(high_low_high, rearrange_method)
        high_high_low = rearrange(high_high_low, rearrange_method)
        high_high_high = rearrange(high_high_high, rearrange_method)
