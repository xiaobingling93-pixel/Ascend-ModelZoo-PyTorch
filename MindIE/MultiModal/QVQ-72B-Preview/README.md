...
# QVQ-72B-Preview
QVQ-72B-Preview 是阿里云研发的大规模视觉语言模型（Large Vision Language Model, LVLM），可以以图像、文本、视频作为输入，并以文本作为输出
## 准备模型
目前提供的MindIE镜像预置了 QVQ-72B-Preview 模型推理脚本，无需使用本仓库自带的atb_models中的代码
## 加载镜像
...
## 硬件要求
部署QVQ-72B-Preview模型至少需要1台Atlas 800I A2 推理服务器 32GB
...
# 模型权重路径
model_path="/data/QVQ-72B-Preview/"
# 批次大小，底层使用continuous batching逻辑
max_batch_size=1
# 最大输入长度，输入长视频或者较大分辨率图片时，需要设置较大的值，以便支持更长的输入序列，QvQ模型最大支持128000
...
"ModelDeployConfig":
{
"maxSeqLen" : 50000, # QvQ模型最大支持128000
"maxInputTokenLen" : 50000, # QvQ模型最大支持128000
"truncation" : false,
"ModelConfig" : [
{
"modelInstanceType": "Standard",
"modelName" : "qwen2_vl", # 为了方便使用benchmark测试，modelname建议使用qwen2_vl
"modelWeightPath" : "/data/datasets/QVQ-72B-Preview",
"worldSize" : 8,
...
