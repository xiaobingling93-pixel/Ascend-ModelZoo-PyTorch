...
AS_LIBRARY = Library("atb", "DEF")


class BaseOpBuilder(ABC):
    _cann_path = None
    ...
    def get_absolute_paths(self, paths):
        ops_path = os.path.abspath(os.path.dirname(os.path.dirname(__file__)))
        return [os.path.join(ops_path, path) for path in paths]
