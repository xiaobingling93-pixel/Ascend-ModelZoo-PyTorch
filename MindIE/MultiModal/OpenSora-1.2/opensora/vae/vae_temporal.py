...
class ResBlock(nn.Module):
    def __init__(
        ...
        self.norm1 = norm_fn(num_groups, in_channels)
        self.conv1 = conv_fn(in_channels, self.filters, kernel_size=(3, 3, 3), bias=False)
        self.norm2 = norm_fn(num_groups, self.filters)
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_out_channels=4, latent_embed_dim=512,  # num channels for latent vector
                 filters=128, num_res_blocks=4, channel_multipliers=(1, 2, 2, 4),
        ...
        # first layer conv
        self._init_first_layer(in_out_channels)

        # ResBlocks and conv downsample
        prev_filters, filters = self._init_res_conv_layer()

        # # last layer res block
        self._init_last_layer(prev_filters, filters)
    ...
    def _init_first_layer(self, in_out_channels):
        ...
        for i in range(self.num_blocks):
            ...
            if i < self.num_blocks - 1:
                if self.temporal_downsample[i]:
                    ...
                    prev_filters = filters  # update in_channels
        return prev_filters, filters

    def _init_last_layer(self, prev_filters, filters):
        # last layer res block
        ...
        self.norm1 = self.norm_fn(self.num_groups, prev_filters)
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in reversed(range(self.num_blocks)):
            ...
            if i > 0:
                ...
                x = self.rearrange_with_reshape(x, t_stride, self.s_stride, self.s_stride)
        x = self.norm1(x)
        x = self.activate(x)
        x = self.conv_out(x)
        return x

    def rearrange_with_reshape(self, x, ts, hs, ws):
        x_shape0_b, c_ts_hs_ws, x_shape2_t, x_shape3_h, x_shape4_w = x.shape
        c = c_ts_hs_ws // (ts * hs * ws)

        x = x.reshape(x_shape0_b, c, ts, hs, ws, x_shape2_t, x_shape3_h, x_shape4_w)
        x = x.permute(0, 1, 5, 2, 6, 3, 7, 4)
        x = x.reshape(x_shape0_b, c, x_shape2_t * ts, x_shape3_h * hs, x_shape4_w * ws)
        return x
    ...
    def _init_layers(self):
            ...
            if i > 0:
                if self.temporal_downsample[i - 1]:
                    ...
                    self.conv_blocks.insert(
                        0,
                        self.conv_fn(
                            prev_filters, prev_filters * t_stride * self.s_stride * self.s_stride,
                            kernel_size=(3, 3, 3)
                        ),
                    )
                else:
        ...
        self.norm1 = self.norm_fn(self.num_groups, prev_filters)
        self.conv_out = self.conv_fn(filters, self.in_out_channels, 3)


class VAETemporal(nn.Module):
    def __init__(self, in_out_channels=4, latent_embed_dim=4, embed_dim=4, filters=128, num_res_blocks=4,
                 channel_multipliers=(1, 2, 2, 4), temporal_downsample=(True, True, False),
                 num_groups=32,  # for nn.GroupNorm
                 activation_fn="swish",
                 ):
        super().__init__()
