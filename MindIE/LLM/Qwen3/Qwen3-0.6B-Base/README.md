...
- [Qwen3-0.6B-Base](https://www.modelscope.cn/models/Qwen/Qwen3-0.6B-Base/files)
...
目前提供的MindIE镜像预置了Qwen3-0.6B-Base模型推理脚本，无需再额外下载魔乐仓库承载的模型适配代码，直接新建容器即可。
...
```
具体执行batch=1, 输入长度256, 输出长度256用例的1卡并行性能测试命令为：
```shell
bash run.sh pa_bf16 performance [[256,256]] 1 qwen ${weight_path} 1
```
...
...
"modelName" : "qwen3",
"modelWeightPath" : "/data/datasets/Qwen3-0.6B-Base",
"worldSize" : 1,
...
}
]
