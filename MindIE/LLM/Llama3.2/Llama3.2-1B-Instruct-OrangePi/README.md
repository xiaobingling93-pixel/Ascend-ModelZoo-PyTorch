# Llama3.2-1B-Instruct-OrangePi

## 约束条件
* 在OrangePi AIpro(20T)上部署Llama3.2-1B-Instruct模型
* 需要修改权重目录下的config.json文件，"torch_dtype"字段改为"float16", "max_position_embedding"字段改为4096（根据输入输出配置调整，大于max_input_length + max_output_length）
* 由于此硬件为单卡，仅支持TP=1
