...
5. ONNX转换om模型
   ...
   1. 对于encoder，需要先进行改图
      ...
      再进行om转换，其中参数soc_version用于指定模型转换时昇腾AI处理器的版本（Ascendxxx中的xxx需要通过npu-smi info命令进行查询）

      ```sh
      atc --input_shape="x:1,100,80;x_lens:1" --precision_mode=force_fp32 --soc_version=Ascendxxx --framework=5 --output=encoder_py310 --model ./simed_encoder_modified.onnx
      ```
   ...
   2. 对于decoder，需要先进行改图
      ...
      ```sh
      atc --input_shape="y:1,2" --precision_mode=force_fp32 --soc_version=Ascendxxx --framework=5 --output=decoder_py310 --model=simed_decoder_modified.onnx
      ```

   3. Joiner模型可以直接转换om，其中参数soc_version用于指定模型转换时昇腾AI处理器的版本（Ascendxxx中的xxx需要通过npu-smi info命令进行查询）

      ```sh
      atc --input_shape="encoder_out:1,512;decoder_out:1,512" --soc_version=Ascendxxx --framework=5 --output=joiner_py310 --model=./exp/joiner-epoch-99-avg-1.onnx
      ```
