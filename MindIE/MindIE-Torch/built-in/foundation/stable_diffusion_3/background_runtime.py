...
class BackgroundRuntime:
    ...
    def run_infer(
        ...
        while sync_pipe.recv() != 'STOP':
            ...
            hidden_states_npu = hidden_states.to(torch.float32).to(f"npu:{device_id}")
            encoder_hidden_states_npu = encoder_hidden_states.to(torch.float32).to(f"npu:{device_id}")
            pooled_projections_npu = pooled_projections.to(torch.float32).to(f"npu:{device_id}")
