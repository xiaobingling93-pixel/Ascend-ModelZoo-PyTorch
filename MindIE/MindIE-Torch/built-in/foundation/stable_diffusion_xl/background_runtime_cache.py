...
class BackgroundRuntime:
    ...
    def run_infer(
        ...
        while True:
            ...
            if flag == 'cache':
                sample, timestep, prompt_embeds, add_text_embeds, add_time_ids, return_flag = [
            ...
            prompt_embeds_npu = prompt_embeds.to(torch.float32).to(f"npu:{device_id}")
            add_text_embeds_npu = add_text_embeds.to(torch.float32).to(f"npu:{device_id}")
            add_time_ids_npu = add_time_ids.to(torch.float32).to(f"npu:{device_id}")
            flag_npu = return_flag.to(torch.int64).to(f"npu:{device_id}")

            if flag == 'cache':
                with mindietorch.npu.stream(stream):
                    output_npu = model_cache(sample_npu, timestep_npu, prompt_embeds_npu, add_text_embeds_npu,
                                             add_time_ids_npu, flag_npu)
                    stream.synchronize()
                ...
                with mindietorch.npu.stream(stream):
                    output_npu = model_skip(sample_npu, timestep_npu, prompt_embeds_npu, add_text_embeds_npu,
                                            add_time_ids_npu, flag_npu, return_cache)
                    stream.synchronize()
