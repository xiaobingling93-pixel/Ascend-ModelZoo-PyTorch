...
class BackgroundRuntime:
    ...
    def run_infer(
        ...
        while sync_pipe.recv() != 'STOP':
            ...
            timestep_npu = timestep.to(torch.float32).to(f"npu:{device_id}")
            encoder_hidden_states_npu = encoder_hidden_states.to(torch.float32).to(f"npu:{device_id}")
            added_time_ids_npu = added_time_ids.to(torch.float32).to(f"npu:{device_id}")

            # print("<<<<<<<<<<<<<<<<sample_npu.shape"f"{sample_npu.shape}>>>>>>>>>>>>>>>>")                                # 1 25 8 72 128
            # print("<<<<<<<<<<<<<<<<timestep_npu.shape"f"{timestep_npu.shape}>>>>>>>>>>>>>>>>")                            # 1
            # print("<<<<<<<<<<<<<<<<encoder_hidden_states_npu.shape"f"{encoder_hidden_states_npu.shape}>>>>>>>>>>>>>>>>")  # 1 1 1024
            # print("<<<<<<<<<<<<<<<<added_time_ids_npu.shape"f"{added_time_ids_npu.shape}>>>>>>>>>>>>>>>>")                # 1 3
            # print("<<<<<<<<<<<<<<<<sample_npu.type"f"{sample_npu.dtype}>>>>>>>>>>>>>>>>")                                 # float32
            # print("<<<<<<<<<<<<<<<<timestep_npu.type"f"{timestep_npu.dtype}>>>>>>>>>>>>>>>>")                             # int64
            # print("<<<<<<<<<<<<<<<<encoder_hidden_states_npu.type"f"{encoder_hidden_states_npu.dtype}>>>>>>>>>>>>>>>>")   # float32
            # print("<<<<<<<<<<<<<<<<added_time_ids_npu.type"f"{added_time_ids_npu.dtype}>>>>>>>>>>>>>>>>")                 # float32


            with mindietorch.npu.stream(stream):
                output_npu = model(sample_npu, timestep_npu, encoder_hidden_states_npu, added_time_ids_npu)
                stream.synchronize()
