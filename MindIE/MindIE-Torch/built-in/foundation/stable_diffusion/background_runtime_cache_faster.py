...
@dataclass
class RuntimeIOInfoCacheFaster:
...
class BackgroundRuntimeCacheFaster:
    def __init__(
            self,
            device_id: int,
            model_path: str,
            io_info: RuntimeIOInfoCacheFaster
    ):
    ...
    def run_infer(
        ...
        while True:
            ...
            if flag == 'cache':
                sample, timestep, encoder_hidden_states, return_flag, return_faster_flag = [
            ...
            faster_flag_npu = return_faster_flag.to(torch.int64).to(f"npu:{device_id}")

            if flag == 'cache':
                with mindietorch.npu.stream(stream):
                    output_npu = model_cache(sample_npu, timestep_npu, encoder_hidden_states_npu, flag_npu, faster_flag_npu)
                    stream.synchronize()
                    ...
                    return_cache_faster = output_npu[2]
            else:
                with mindietorch.npu.stream(stream):
                    output_npu = model_skip(sample_npu, timestep_npu, encoder_hidden_states_npu, flag_npu, faster_flag_npu, return_cache, return_cache_faster)
                    stream.synchronize()
    ...
    @classmethod
    def clone(cls, device_id: int, model_path: str, runtime_info: RuntimeIOInfoCacheFaster) -> 'BackgroundRuntimeCacheFaster':
