...
class AIEStableDiffusionPipeline(StableDiffusionPipeline):
    ...
    def compile_aie_model(self):
        ...
        clip_compiled_path = os.path.join(self.args.output_dir, f"clip/clip_bs{size}_aie_compile.ts")
        if os.path.exists(clip_compiled_path):
            self.compiled_clip_model = torch.jit.load(clip_compiled_path).eval()
        else:
            model = torch.jit.load(os.path.join(self.args.output_dir, f"clip/clip_bs{size}.pt")).eval()
        ...
        if os.path.exists(vae_compiled_path):
            ...
            model = torch.jit.load(os.path.join(self.args.output_dir, f"vae/vae_bs{size}.pt")).eval()
