...
def export_clip(sd_pipeline: StableDiffusionPipeline, save_dir: str, batch_size:int) -> None:
    ...
    if os.path.exists(os.path.join(clip_path, "clip.onnx")):
        return
...
def export_unet(sd_pipeline: StableDiffusionPipeline, save_dir: str, batch_size: int) -> None:
    ...
    if os.path.exists(os.path.join(unet_path, "unet.onnx")):
        return
...
def export_vae(sd_pipeline: StableDiffusionPipeline, save_dir: str, batch_size: int) -> None:
    ...
    if os.path.exists(os.path.join(vae_path, "vae.onnx")):
        return
...
def main():
    ...
    model = os.path.join(cur_dir_path, "models/stable-diffusion-2-1-base")
    
    parallel = False
    output_dir = os.path.join(cur_dir_path, "models/SD2.1/models_bs1")
    export_onnx(model, output_dir, args.batch_size, parallel)
    parallel = True
    output_dir = os.path.join(cur_dir_path, "models/SD2.1/models_bs1_parallel")
    export_onnx(model, output_dir, args.batch_size, parallel)
    print("Done.")
