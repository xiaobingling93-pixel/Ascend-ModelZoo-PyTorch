...
class BackgroundRuntime:
    ...
    def run_infer(
        ...
        while sync_pipe.recv() != 'STOP':
            ...
            x_npu = x.to(torch.float32).to(f"npu:{device_id}")
            t_npu = t.to(torch.int64).to(f"npu:{device_id}")
            y_npu = y.to(torch.int64).to(f"npu:{device_id}")
            ...
            with mindietorch.npu.stream(stream):
                inf_start = time.time()
                output_npu = model(x_npu, t_npu, y_npu)
                stream.synchronize()
                inf_end = time.time()
