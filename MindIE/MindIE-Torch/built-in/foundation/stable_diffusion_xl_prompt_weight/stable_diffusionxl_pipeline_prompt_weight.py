...
from diffusers.utils import make_image_grid, USE_PEFT_BACKEND
...
class PromptLoader:
    def __init__(self, prompt_file: str, prompt_file_type: str, batch_size: int,
                 num_images_per_prompt: int = 1, max_num_prompts: int = 0
    ):
        self.prompts = []
    ...
    def __next__(self):
        ...
        ret = {'prompts': [], 'catagories': [], 'save_names': [], 'n_prompts': self.batch_size,}
...
class AIEStableDiffusionXLPipeline(StableDiffusionXLPipeline):
    ...
    def encode_prompt(self, prompt: str, prompt_2: Optional[str] = None, num_images_per_prompt: int = 1,
            do_classifier_free_guidance: bool = True, negative_prompt: Optional[str] = None,
            negative_prompt_2: Optional[str] = None, prompt_embeds: Optional[torch.FloatTensor] = None,
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None and zero_out_negative_prompt:
            ...
            for negative_prompt, tokenizer, text_encoder in zip(uncond_tokens, tokenizers, text_encoders):
                ...
                uncond_input = tokenizer(negative_prompt, padding="max_length", max_length=max_length,
                    truncation=True, return_tensors="pt",)
    ...
    @torch.no_grad()
    def ascendie_infer(self, prompt: Union[str, List[str]] = None, prompt_2: Optional[Union[str, List[str]]] = None,
            height: Optional[int] = None, width: Optional[int] = None, num_inference_steps: int = 50,
            timesteps: List[int] = None, denoising_end: Optional[float] = None,
            guidance_scale: float = 7.5, negative_prompt: Optional[Union[str, List[str]]] = None,
        ...
        ) = self.encode_prompt(  # nv
            prompt=prompt,  # None
            prompt_2=prompt_2,  # None
        ...
        add_text_embeds = add_text_embeds.to(device)  # [4,1280]
        add_time_ids = add_time_ids.to(device).repeat(batch_size * num_images_per_prompt, 1)  # [4,6]
        ...
        for i, t in enumerate(timesteps):
            ...
            if latents.is_cpu:
                latent_model_input = torch.cat([latents] * 2)  # latent_model_input:[4,4,128,128]

                # latent_model_input:[4,4,128,128]
            ...
            if flag_cache:
                ...
                noise_pred = self.compiled_unet_model(latent_model_input,  # [4,4,128,128]
                                                      t.to(torch.int64)[None].to(f'npu:{self.device_0}'),  # [1]
                                                      prompt_embeds.to(f'npu:{self.device_0}'),  # [4,77,2048]
                                                      add_text_embeds.to(f'npu:{self.device_0}'),  # [4,1280]
                                                      add_time_ids.to(f'npu:{self.device_0}'))  # [4,6]
            unet_time += time.time() - start

            # perform guidance
            if do_classifier_free_guidance:
                if flag_ddim:
                    if not noise_pred.is_cpu:
