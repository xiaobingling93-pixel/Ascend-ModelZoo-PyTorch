...
- 输入数据
  ...
  | 输入数据 | 数据类型 | 大小                      | 数据排布格式 |
  | -------- | -------- | ------------------------- | ------------ |
  | input_ids    | INT64 | batchsize x seqlen | ND         |
  | position_ids    | INT64 | batchsize x seqlen | ND         |
  | attention_mask    | INT64 | batchsize x seqlen | ND         |
  | past_key_values    | FLOAT | batchsize x 2 x seqlen x batchsize x 2 x 128 | ND         |
...
2. 开始对话验证。
   ```
   python3 example.py --device=npu --batch_size=1
   ```
