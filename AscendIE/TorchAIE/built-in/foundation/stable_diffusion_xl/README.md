...
1. 模型转换。
   ...
   1. 导出pt模型并进行编译。(可选)
      ...
      # 若使用unetCache
      python3 export_ts_cache.py --model ${model_base} --output_dir ./models_unetCache
      ...
      不使用unetCache，执行成功后会生成pt模型：
         - ./models/clip/clip.pt  和 ./models/clip/clip_2.pt
         - ./models/unet/unet_bs2.pt
         - ./models/vae/vae.pt
      
      使用unetCache，执行成功后会生成pt模型：
         - ./models_unetCache/clip/clip.pt  和 ./models_unetCache/clip/clip_2.pt
         - ./models_unetCache/unet/unet_bs2_0.pt 和 ./models_unetCache/unet/unet_bs2_1.pt
         - ./models_unetCache/vae/vae.pt
...
2. 开始推理验证。
   ...
   1. 执行推理脚本。
      ...
      # 使用UnetCache策略
      python3 stable_diffusion_pipeline_cache.py \
              --model ${model_base} \
              --prompt_file ./prompts.txt \
              --device 0 \
              --save_dir ./results_unetCache \
              --steps 50 \
              --output_dir ./models_unetCache
      ```
      ...
      ```
      [info] infer number: 16; use time: 104.6s; average time: 13.107s
      ```
      

      带unetCache策略，执行完成后在`./results_unetCache`目录下生成推理图片。并在终端显示推理时间，参考如下：
      ```
      [info] infer number: 16; use time: 104.6s; average time: 6.033s
      ```
