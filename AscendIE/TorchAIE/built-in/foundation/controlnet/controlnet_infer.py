...
def parse_arguments():
    ...
    parser.add_argument("--prompt", type=str, default="cute dog", help="label=Prompt")
    parser.add_argument(
        "--a_prompt",
        type=str,
    ...
    parser.add_argument("--num_samples", type=int, default=1, help="image_num")
    parser.add_argument(
        "--image_resolution",
        type=int,
        nargs=2,
    ...
    parser.add_argument(
        "--control_model_dir",
        type=str,
        default="./models/control/control.pt",
        help="path of pt model.",
    )
    parser.add_argument(
        "--sd_model_dir",
        type=str,
        default="./models/sd/sd.pt",
        help="path of pt model.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save_dir", type=str, default="./results", help="Path to save result images."
    )
    parser.add_argument(
        "--ddim_steps", type=int, default=20, help="Number of inference steps."
    )
...
def main():
    ...
    if os.path.exists(args.compiled_control_model_dir) and os.path.exists(
        ...
        model.compiled_control_model = torch.jit.load(
            args.compiled_control_model_dir
        ).eval()
        model.compiled_sd_model = torch.jit.load(args.compiled_sd_model_dir).eval()
    else:
        model.compile_pt_model(args.control_model_dir, args.sd_model_dir)
