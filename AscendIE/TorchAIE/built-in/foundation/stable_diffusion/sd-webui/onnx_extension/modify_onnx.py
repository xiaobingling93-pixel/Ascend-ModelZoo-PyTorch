...
def main():
    ...
    model_path = os.path.join(cur_dir_path, "models/SD2.1/models_bs1/unet/unet.onnx")
    new_model = os.path.join(cur_dir_path, "models/SD2.1/models_bs1/unet/unet_md.onnx")
    ...
    model_parallel_path = os.path.join(cur_dir_path, "models/SD2.1/models_bs1_parallel/unet/unet.onnx")
    new_model = os.path.join(cur_dir_path, "models/SD2.1/models_bs1_parallel/unet/unet_md.onnx")
    if not os.path.exists(new_model):
        model_parallel = OnnxGraph.parse(model_parallel_path)
        del_add(model_parallel)
        if NpuConfig.Duo:
            add_flash_attention(model_parallel, 'FlashAttentionTik', soc_type=1)
        elif NpuConfig.A2:
            add_flash_attention(model_parallel, 'UnpadFlashAttentionMix', soc_type=2)
        if args.TOME_num:
            insert_tome_block(model_parallel, args.TOME_num)
        change_input_type(model_parallel)
        replace_slice(model_parallel, args.faster_gelu)
        model_parallel.remove_unused_nodes()
        new_model = os.path.join(cur_dir_path, "models/SD2.1/models_bs1_parallel/unet/unet_md.onnx")
        model_parallel.save(new_model)
