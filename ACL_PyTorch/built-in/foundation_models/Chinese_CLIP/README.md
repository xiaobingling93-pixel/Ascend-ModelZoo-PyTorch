...
3. 安装离线推理所需依赖。
   ...
   点击下载 [torch_npu](https://gitee.com/ascend/pytorch/releases/download/v5.0.1-pytorch1.11.0/torch_npu-1.11.0.post9-cp38-cp38-linux_aarch64.whl) 。

   ```shell
   pip3 install -U pip && pip3 install -r requirements.txt
   pip3 install torch_npu-1.11.0.post9-cp38-cp38-linux_aarch64.whl
   ```

4. 安装改图工具 [auto-optimizer](https://gitee.com/ascend/msadvisor/tree/master/auto-optimizer)。
...
1. 获取原始数据集。
   ...
   本文档采用 ELEVATER 中的 CIFAR-100 数据集验证精度，点击[下载](http://clip-cn-beijing.oss-cn-beijing.aliyuncs.com/datasets/cifar-100.zip)处理好的数据集，解压后放置于任意目录下 `${path_to_dataset}`。

2. 数据集目录结构。

   ```
   ${path_to_dataset}/cifar-100
   |-- index.json
   |-- label_cn.txt
   |-- label.txt
   |-- test
...
1. 模型转换。
   ...
   2. 导出onnx文件。
      ...
      1. 使用 Chinese-CLIP/cn_clip/deploy/pytorch_to_onnx.py 导出onnx文件。
         ...
         运行成功后，使用models目录下生成的 vit-b-16.img.fp16.onnx 文件进行后续操作。
   ...
   3. 使用 opt_onnx.py 优化 onnx 模型。
   
      ```shell
      # 例如 export bs=16
      python3 opt_onnx.py models/vit-b-16.img.fp16.bs${bs}.sim.onnx models/vit-b-16.img.fp16.bs${bs}.opt.onnx vit_base_patch16_224
      ```
   ...
   3. 使用ATC工具将ONNX模型转OM模型。
      ...
      3. 执行ATC命令。
         ```shell
         # 例如 export bs=16 && export chip_name=310P3
         atc --framework=5 \
...
2. 开始推理验证。
   ...
   2. 精度验证。
      ...
      ```shell
      python3 evaluate.py \
      --datapath ${path_to_dataset}/cifar-100/test/ \
      --dataset cifar-100 \
      --label-file ${path_to_dataset}/cifar-100/label_cn.txt \
      --save-class-embeddings
      ```
   
      - 参数说明：
        - --datapath：数据集所在路径，这里为cifar-100的测试集。
        - --dataset：数据集名称。
        - --label-file：数据集真值标签。
        - --save-class-embedding：是否保存分类器权重。
   
      运行成功后，在 models 目录下生成 classifier.pt 权重文件。
   
      b. 运行如下命令，进行图像分类任务的推理：
      ...
      --label-file ${path_to_dataset}/cifar-100/label_cn.txt \
      --img-batch-size ${bs} \
      --img-model-path models/vit-b-16.img.fp16.bs${bs}.om
      ```
      ...
      - 参数说明：
        ...
        - --img-batch-size：图像批大小。
        - --img-model-path：离线推理模型文件的路径。
   
      运行成功后打印离线模型推理耗时、端到端耗时和 top1 精度.
...
调用ACL接口推理计算，模型性能和精度参考下列数据。

| 芯片型号  | Batch Size | 数据集精度 | 纯推理性能 | 单次推理耗时 |
|----------|------------|----------|-------|----------|
|  310P3  |       1       | 63.91% | 327.51fps | 5.6ms |
| 310P3 | 16 | 63.89% | 546.35fps | 43.6ms |
| 310P3 | 64 | 63.58% | 481.75fps | 215.2ms |
