...
1. 模型转换。
   ...
   2. 导出onnx文件。
      ...
      1. 移动DiT_pt2onnx.py至DiT目录，使用DiT_pt2onnx.py到处onnx文件。
      ...
      2. 使用onnxsim精简onnx文件
         ...
         ```
         python3 -m onnxsim ./models/onnx/dit_dynamic_512.onnx ./models/onnx/dit_dynamic_512_sim.onnx
         ```

         获得dit_dynamic_512_sim.onnx文件。
   ...
   3. 使用ATC工具将ONNX模型转OM模型。
      ...
      3. 执行ATC命令。
         ...
         ```
         batch_size=16
         atc --model=./models/onnx/dit_dynamic_512_sim.onnx \
             --framework=5 \
             --output=./models/om/dit_bs${batch_size}_512 \
             --input_format=NCHW \
...
2. 开始推理验证。
   ...
   2. 执行推理。
      ...
      移动sample_npu.py至DiT目录，进行模型推理。

      ```
      mv ./sample_npu.py ./DiT/
      python3 ./DiT/sample_npu.py --image_size 512 --model ./models/om/dit_bs${batch_size}_512.om
      ```
   ...
   3. 精度验证。
      ...
      由于生成图片存在随机性，所以精度验证将使用[ADM's Tensordlow evaluation suite](https://github.com/openai/guided-difffusion/tree/main/evaluations)进行FID和Inception Score计算。

      注意，由于要生成图片数量较多，进行完整的精度验证需要耗费很长的时间。
      移动sample_ddp_npu.py至DiT目录，进行模型并行推理。

      ```
      mv ./sample_ddp_npu.py ./DiT/
      python3 ./DiT/sample_ddp_npu.py --image_size 512 --model ./models/om/dit_bs${batch_size}_512.om --per_proc_batch_size ${batch_size}
      ```
      ...
      - 参数说明：
        ...
        -   --model: om文件路径。
        -   --vae: 图片解码模式。
        -   --per\_proc\_batch\_size: batch大小。
        -   --image\_size: 输入图片大小。
        -   --num\_classes: 图片类别数量。
        -   --num\_sampling\-steps: 每批次输入做推理的次数。
        -   --num\_fid\_samples: 需要测试的样本数量。
        -   --device\_id： NPU设备编号。
