...
本模型采用COCO2105数据集的测试集（test）进行精度评估。请获取[coco官方数据集](https://images.cocodataset.org/zips/test2015.zip)并解压后将test2015文件夹放在当前文件夹下。目录结构如下：
...
1. 模型转换。
   ...
   2. 导出onnx文件。
      ...
      2. 固定 shape 和 simplify。

         ```bash
         bs=32
         seq_len=35
         k_test=128 # k_test 与配置文件一致，默认为 128

         for model in 'visual_encoder' 'text_encoder' 'text_decoder_1' 'text_decoder_2'; do
           python3 -m onnxruntime.tools.make_dynamic_shape_fixed \
             --dim_param bs \
             --dim_value "${bs}" \
             "blip_models/${model}.onnx" \
             "blip_models/${model}_sim.onnx"
           python3 -m onnxruntime.tools.make_dynamic_shape_fixed \
             --dim_param seq_len \
             --dim_value "${seq_len}" \
             "blip_models/${model}_sim.onnx" \
             "blip_models/${model}_sim.onnx"
           if [[ "${model}" = 'text_decoder_2' ]]; then
             python3 -m onnxruntime.tools.make_dynamic_shape_fixed \
               --dim_param k_test \
               --dim_value "$((k_test*bs))" \
               "blip_models/${model}_sim.onnx" \
           ...
           onnxsim "blip_models/${model}_sim.onnx" "blip_models/${model}_sim.onnx"
         done
         ```
      ...
      3. 优化ONNX文件。
         ...
         运行modify_onnx.py脚本。
         ```shell
         python3 modify_onnx.py --model_dir blip_models
         ```
         - 参数说明：
            - --model_dir：存放模型的目录，默认为 blip_models。
   ...
   3. 使用ATC工具将ONNX模型转OM模型。
      ...
      3. 执行ATC命令。
         ...
         atc \
           ...
           --output blip_models/visual_encoder_md \
         ...
         atc \
           ...
           --output blip_models/text_encoder_md \
           --soc_version "Ascend${chip_name}"

         atc \
           --framework 5 \
           --model blip_models/text_decoder_1_md.onnx \
           --output blip_models/text_decoder_1_md \
           --soc_version "Ascend${chip_name}"
             
         atc \
           --framework 5 \
           --model blip_models/text_decoder_2_md.onnx \
           --output blip_models/text_decoder_2_md \
         ...
         ├── blip_models
            ...
            ├── text_encoder_md.om
            ├── text_decoder_1_md.om
            └── text_decoder_2_md.om
         ```
...
| 模型 | 硬件型号 | Batch Size | 性能 |
| ---- | ---- | ---- | ---- |
| 端到端 | Atlas 300I Pro | 32 | 9.87 data/s |
| visual_encoder | Atlas 300I Pro | 32 | 87.93 fps |
| text_encoder | Atlas 300I Pro | 32 | 729.18 fps |
| text_decoder_1 | Atlas 300I Pro | 32 | 4539.00 fps |
| text_decoder_2 | Atlas 300I Pro | 32 | 1708.29 fps |
