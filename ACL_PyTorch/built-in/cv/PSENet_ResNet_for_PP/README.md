...
2. 数据预处理。
   ...
   在工作目录下，执行PSENet_ResNet_preprocess.py脚本，完成预处理。

   ```
    python3 PSENet_ResNet_preprocess.py \
...
1. 模型转换。
   ...
   3. 使用ATC工具将ONNX模型转OM模型。
      ...
      3. 执行ATC命令。
         ...
         ```shell
         atc --framework=5 \
             --model=./PSENet_ResNet_md_dybs.onnx \
             --output=./PSENet_ResNet_bs${bs} \
             --input_format=NCHW \
             --input_shape="x:${bs},3,736,1312" \
             --log=error \
             --soc_version=Ascend${chip_name} \
             --insert_op_conf=PSENet_ResNet_aipp.cfg
         ```
         ...
         - 参数说明：
           ...
           运行成功后生成`PSENet_ResNet_bs${bs}.om`模型文件。
...
2. 开始推理验证。
   ...
   3. 精度验证。
      ...
      执行后处理脚本`PSENet_ResNet_postprocess.py`，参考命令如下：

      ```
      python3 PSENet_ResNet_postprocess.py \
        -c PaddleOCR/configs/det/det_r50_vd_pse.yml \
   ...
   4. 性能验证。
      ...
      ```
      python3 -m ais_bench --model=./PSENet_ResNet_bs${bs}.om --loop=50
      ```
