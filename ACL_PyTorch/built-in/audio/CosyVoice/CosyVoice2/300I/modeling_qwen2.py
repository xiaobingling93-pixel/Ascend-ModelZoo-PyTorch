...
class Qwen2SdpaAttention(Qwen2Attention):
    ...
    def group_mm_torch(self, heads, group_num, A, B):
        ...
        for i in range(group_num):
            group_score = torch.matmul(A[i * group_head: (i + 1) * group_head, :, :], B[i: (i + 1), :, :])
            if score is None:
                score = group_score
            else:
    ...
    def forward(
        ...
        key_states = key_states.transpose(1, 2)  # BNSD
        ...
        if q_len > 1:
            ...
            for idx in range(bsz):
                q_slice = query_states[idx]
                k_slice = key_states[idx].transpose(1, 2)
                v_slice = value_states[idx]
                score = self.group_mm_torch(self.num_heads, self.num_key_value_heads, q_slice, k_slice)
                score = score * self.scale
                score = score + attention_mask[idx]
                score = F.softmax(score, dim=-1, dtype=torch.float32).to(q_slice.dtype)
                out = self.group_mm_torch(self.num_heads, self.num_key_value_heads, score, v_slice).unsqueeze(0)
                if attn_output is None:
                    attn_output = out
                else:
                    attn_output = torch.cat((attn_output, out), dim=0)
        ...
        if use_cache:
            ...
            if use_legacy_cache:
                kv_shape = (
                    ...
                    self.config.hidden_size // self.config.num_attention_heads)     # (1, 2, 32768, 64)
                past_key_values = ()
...
class Qwen2ForCausalLM(Qwen2PreTrainedModel):
    ...
    def forward(
        ...
        updated_kv_positions, past_key_values, position_ids, actual_seq_len = self.prepare_data(inputs_embeds, past_key_values, prompt_length)
    ...
    def prepare_data(self, inputs_embeds, past_key_values, prompt_length):
        ...
        return updated_kv_positions, past_key_values, position_ids, actual_seq_len
