...
3. 导出om模型
   ```
   atc --model=./PP-OCRv5_server_det_infer/inference_opt.onnx --framework=5 --output=./PP-OCRv5_server_det_infer/inference --soc_version=Ascend310P3 --input_shape "x:-1,3,-1,-1"
   atc --model=./PP-OCRv5_server_rec_infer/inference_opt.onnx --framework=5 --output=./PP-OCRv5_server_rec_infer/inference --soc_version=Ascend310P3 --input_shape "x:-1,3,48,-1"
   ```
### 运行推理脚本
...
对于文本检测模型，输入的数据要求长宽为32的整数倍，我们以（1,3,960,960）的输入shape为例，运行如下命令测试推理性能，
   ```
   python -m ais_bench --model ../PP-OCRv5_server_det_infer/inference_linux_aarch64.om --outputSize "100000000" --dymShape "x:1,3,960,960"
   ```
对于文本识别模型，以（1,3,48,320）shape为例，运行如下命令测试性能
   ```
   python -m ais_bench --model ../PP-OCRv5_server_rec_infer/inference_linux_aarch64.om --outputSize "100000000" --dymShape "x:1,3,48,320"
   ```

## 结果展示
   文本检测模型和文本识别模型在示例数据集上的的精度和性能数据展示在下表，其中文本检测模型的精度指标为hmeans，文本识别模型的精度指标为acc：

   |模型|芯片|hmeans/acc|hmeans/acc（A10）|性能（ms）|
   |------|------|------|------|------|
   |PP-OCRv5_server_det|300I DUO|52.87%|52.87%|35.01 | 
   |PP-OCRv5_server_rec|300I DUO|72.27%|72.27%|2.63 |
