...
def parse_args():
    parser = argparse.ArgumentParser(description="BGE-M3 infer")
    ...
    parser.add_argument("--devices", type=str, default="['npu:0']", help="target npu devices")
    args = parser.parse_args()
    return args


def create_model(args):
    model = BGEM3FlagModel("./", trust_remote_code=True)
...
def modify_model(model):
    xlm_roberta_config = model.model.model.config
    xlm_roberta_embeddings = model.model.model.embeddings
    model.model.model.embeddings = MyXLMRobertaEmbeddings(xlm_roberta_config)
    model.model.model.embeddings.load_state_dict(xlm_roberta_embeddings.state_dict())
    model.model.model.embeddings.eval().half()

    for layer in model.model.model.encoder.layer:
...
if __name__ == '__main__':
    ...
    sentences1 = ["What is BGE M3?", "Defination of BM25"]
    sentences2 = [
        "BGE M3 is an embedding model supporting dense retrieval, lexical matching and multi-vector interaction.",
        "BM25 is a bag-of-words retrieval function that ranks a set of documents based on the query terms appearing i each document"
    ]

    with torch.inference_mode():
        for _ in range(args.warmup):
            output1 = model.encode(sentences1, return_dense=True, return_sparse=True, return_colbert_vecs=True)
            output2 = model.encode(sentences2, return_dense=True, return_sparse=True, return_colbert_vecs=True)

            dense_vecs1 = output1['dense_vecs']
            dense_vecs2 = output2['dense_vecs']
            print("sentences1 = {}".format(sentences1))
            print("sentences2 = {}".format(sentences2))
            print(" dense similarity scores = {}".format(dense_vecs1 @ dense_vecs2.T))

            lexical_weights1 = output1['lexical_weights']
            lexical_weights2 = output2['lexical_weights']
            print("lexical_weights for sentences1 = {}".format(model.convert_id_to_token(lexical_weights1)))
            print("lexical_weights for sentences2 = {}".format(model.convert_id_to_token(lexical_weights2)))
            lexical_similarity = model.compute_lexical_matching_score(lexical_weights1, lexical_weights2)
            print("lexical similarity scores = {}".format(lexical_similarity))

            multi_vecs1 = output1['colbert_vecs']
            multi_vecs2 = output2['colbert_vecs']
            multi_vecs_similarity = [
                model.colbert_score(multi_vecs1[0], multi_vecs2[0]),
                model.colbert_score(multi_vecs1[0], multi_vecs2[1]),
                model.colbert_score(multi_vecs1[1], multi_vecs2[0]),
                model.colbert_score(multi_vecs1[1], multi_vecs2[1]),
            ]
            print("multi vecs similarity scores = {}".format(multi_vecs_similarity))
