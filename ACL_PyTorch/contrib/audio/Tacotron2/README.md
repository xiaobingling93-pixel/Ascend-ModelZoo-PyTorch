...
1. 模型转换。
   ...
   2. 导出onnx文件。
      ...
      3. 使用pth导出onnx。
            ...
            ```
            cd $tacotron_path 
            python3.7 tensorrt/convert_tacotron22onnx.py --tacotron2 ./checkpoints/nvidia_tacotron2pyt_fp32_20190427 --output output/ --batch_size 4 --iter 1
            cd output
            mkdir iter
            mv *.weight 3* iter/
            ```
         ...
         >注意使用不同版本的onnx生成的权重节点名称可能不同，请自行更改上述生成的权重名称。
            ...
            ```
            cd $tacotron_path
            python3.7 tensorrt/convert_tacotron22onnx.py --tacotron2 ./checkpoints/nvidia_tacotron2pyt_fp32_20190427 --output output/ --batch_size 4 --iter 100
            cd output
            rm -rf 2*
            ```
         - 参数说明：
          ...
          - --iter：层数。
           ...
           >注意该步骤生成的权重占用磁盘空间约7GB，请注意预留空间。
            ...
            ```
            cd $tacotron_path
            python3.7 tensorrt/convert_waveglow2onnx.py --waveglow ./checkpoints/nvidia_waveglowpyt_fp32_20190427 --config-file config.json --output output/
            ```
            - 参数说明：
...
| 模型            | 300I Pro性能   | T4性能     |
|-----------------|----------|----------|
| Tacotron2(bs1)  | 2942.48  | 606.36   |
| Tacotron2(bs4)  | 9808.69  | 2431.74  |
| Tacotron2(bs8)  | 18891.84 | 4312.98  |
| Tacotron2(bs16) | 33508.43 | 9040.19  |
| Tacotron2(bs32) | 8965.99  | 11328.52 |
| 最优bs          | 33508.43 | 11328.52 |
> **须知：** 
