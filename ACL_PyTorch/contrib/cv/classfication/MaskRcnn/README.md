...
2. 数据预处理  
    ...
    ```
    参数说明：
     + --image_src_path：处理前原数据集的地址
     + --bin_file_path：生成数据集的文件夹名称
     + --model_input_height：模型输入数据高度
     + --model_input_width：模型输入数据宽度
...
3. 生成数据集信息文件
    ...
    使用get_info.py脚本，输入已经得到的二进制文件，输出生成二进制数据集的info文件。运行成功后，在当前目录中生成maskrcnn.info。之后JPG图片info文件生成,运行成功后，在当前目录中生成maskrcnn_jpeg.info。
...
2. ONNX 模型转 OM 模型  
    ...
    # 执行 ATC 进行模型转换
    atc --model=model.onnx --framework=5 --output=maskrcnn_detectron2_npu --input_format=NCHW --input_shape="0:1,3,1344,1344" --out_nodes="Cast_1673:0;Gather_1676:0;Reshape_1667:0;Slice_1706:0" --log=error --soc_version=Ascend${chip_name}
    ```
...
3. 精度验证  
    ...
    ```bash
    python3 maskrcnn_pth_postprocess_detectron2.py --bin_data_path=./results/result/ --test_annotation=maskrcnn_jpeg.info --det_results_path=./ret_npuinfer/ --net_out_num=4 --net_input_height=1344 --net_input_width=1344 --ifShowDetObj
    ```
    参数说明：
...
在设备上，OM模型的精度为  **{AP@50:53.714}**，当batchsize设为1时模型性能最优，纯推理达 22.06 fps。

| 芯片型号   | BatchSize | 数据集      | 精度            | 纯推理性能       | coco2017验证集性能       |
| --------- | --------- | ----------- | --------------- | --------- | --------- |
|300I Pro| 1         | COCO2017  | AP@50:53.714 | 22.06 fps | 13.31 fps |
