...
2. 获取源码。**<u>*此处获取指获取第三方开源代码仓的命令*</u>**
   ...
   如果安装 mmcv-full 报错 undefined symbol，可以尝试 [官方](https://mmcv.readthedocs.io/zh-cn/v1.4.1/get_started/installation.html) 提供的下载源：
   
   ```bash
   pip3 install mmcv-full==1.3.7 -f https://download.openmmlab.com/mmcv/dist/cpu/torch1.5.0/index.html
   ```
   ...
   2. 导出onnx文件。
      ...
      2. 优化ONNX文件。
         ...
         - 参数说明：
            - 第一个参数：待优化模型
            - 第二个参数：优化后模型
            - --input-shape：模型输入shape
            - --dynamic-input-shape：动态模型输入
