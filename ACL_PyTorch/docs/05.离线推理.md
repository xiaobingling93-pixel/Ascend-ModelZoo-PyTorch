...
前面讲到，在线推理的端到端流程也包含预处理、执行推理、后处理这三个关键步骤。关于这三个步骤的作用，请移步[在线推理](02.在线推理.md)章节学习，这里不再赘述。离线推理也包含这三个步骤，为了减轻离线推理代码适配的工作量以及更好的体会各步骤的差异，我们在`PytorchInferencer`类的基础上实现`OmInferencer`，仅做最小的修改。
